    ...
    return async function register(app) {
        ...
        app.get('/api/products', { preHandler: requireAuth }, async (req) => {
            ...
            const rows = await db
                ...
                .from(products)
                .orderBy(asc(products.sortOrder), asc(products.name));
            return rows.filter((p) => (includeInactive || p.active) &&
        ...
        app.post('/api/products', { preHandler: requireAdmin }, async (req, reply) => {
            ...
            const category = (await db.select().from(categories).where(eq(categories.id, categoryId)).limit(1))[0];
            if (!category)
                return reply.code(400).send({ error: 'unknown_category' });
            const sortOrder = typeof body?.sortOrder === 'number' ? body.sortOrder : 0;
            const created = (await db.insert(products).values({ name, priceCents, categoryId, sortOrder }).returning())[0];
            return reply.code(201).send(created);
        });
