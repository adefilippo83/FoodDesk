...
import { imageBuffer } from '../settings.js';
/**
...
function drawBackground(doc, s, pageW, pageH) {
...
function drawCancelledStamp(doc, lang, pageW, pageH) {
 ...
 */
async function render(s, cancelled, layout) {
    ...
    layout(doc, d);
    if (cancelled)
        drawCancelledStamp(doc, s.pdfLang, d.pageW, pageH);
    doc.end();
    return done;
}
function drawLogo(doc, s, d) {
    if (!s.logoImage)
        return;
 ...
 */
export function renderKitchenTicket(order, items, s) {
...
export function renderReceipt(order, items, s) {
    const L = LABELS[s.pdfLang];
    ...
    return render(s, order.cancelledAt !== null, (doc, d) => {
        ...
        doc
            .font('Helvetica')
            .fontSize(10)
            .text(`${L.order} #${ticketNo(order)}${order.customerName ? ` · ${order.customerName}` : ''}`, { align: 'center' })
            .text(`${order.serviceDay} · ${timeOf(order)}`, { align: 'center' });
        dashes(doc, d);
        const priceW = Math.min(22 * MM, d.innerW * 0.25);
        const line = (label, amountCents, opts) => {
            const y = doc.y;
            doc
