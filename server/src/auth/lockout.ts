...
type Entry = { count: number; blockedUntil: number; lastFailure: number }
...
function prune(now: number) {
  ...
  for (const [k, e] of attempts) {
    if (e.blockedUntil < now && now - e.lastFailure > BLOCK_MS) attempts.delete(k)
  }
}

export function isBlocked(ip: string, username: string): boolean {
...
export function recordFailure(ip: string, username: string): boolean {
  const now = Date.now()
  prune(now)
  ...
  if (now - entry.lastFailure > BLOCK_MS) entry.count = 0
  entry.count += 1
